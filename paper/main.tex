...
lacking $s_t$, it cannot tell which one the synthase realised. The conformational coordinate that collapses
Gate-3 for citrinin is the same coordinate that would prune the ordering space to nature's single
trajectory---so the C$>$20 coverage tail and the label-symmetric identifiability frontier are one missing
observable, reached from two directions. A pocket lever that generalises past $n{=}1$ would do double duty:
complete the map and break the wall at once.
