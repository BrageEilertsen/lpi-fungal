...
and the rung-4 mixture route is closed by the regiospecific zearalenone TE, leaving rungs~2--3 at a cost
not yet measured); and the $178$ of $205$ ($87\%$) that touch a hard family are exactly the
...
macrolactonization & resorcylic-acid lactones, macrocycles & rung 2--3 (located; cost open) \\
...
(\S\ref{sec:modality-cells}). \textbf{Pre-registered (design):} the $(\text{trajectory},\text{trajectory})$
cell---the product-neutral iteration-grammar degeneracy the duality points at---is empty by construction in
the reachable neighbourhood, because the chemistry that would populate it lies beyond current coverage; its
first occupant is pre-registered, gated on a curated resorcylic aromatisation fold---not yet
registered, as the reachable orsellinic-type cores already are---together with the macrolactonisation
release now implemented: a located, rung-2 coverage gap (\S\ref{sec:modality-cells},
\S\ref{sec:limits}). \textbf{Not a cost artifact:} the engineerable--frontier split is independent of the
