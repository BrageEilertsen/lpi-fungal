...
fungal PKS in MIBiG~4.0, exact reconstruction reaches $9/326$---not a disappointment but the
\emph{boundary} of the natural manifold the engine operates over (a tailoring-latent reformulation
verifies $7/107$ clusters; where a core is producible, tailoring resolves near-uniquely)---and a cross-taxa transfer probe locates its one
...
learnable) versus \emph{compressed}, state-dependent routing through one reusable manifold. A corpus-wide coverage census then makes that boundary
defensible rather than asserted: a cap-lifted re-scan shows it genuine ($0$ of $91$ large cores
reachable, not a scan artifact), and the out-of-grammar remainder sorts onto a \emph{sound-expressibility}
ladder on which $178$ of the $205$ in-scope cores ($87\%$) touch a family whose sound expressibility is
unproven---so sound extension with no new regiochemistry adds no further buildable core, and ``unsolved''
becomes a located map of which
gaps are buildable, which are open research, and which need a different grammar.
