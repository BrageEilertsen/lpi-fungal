...
\textsc{out-of-grammar}. Across all $326$ fungal PKS in MIBiG~4.0, exact program reconstruction
reaches $9/326$---the \emph{boundary} of what the current operator set soundly reaches, not a low
score. A corpus-wide census makes that boundary defensible and sorts the remainder onto a
\emph{sound-expressibility} ladder, turning ``unsolved'' into a located map: which gaps are
buildable, which are open research, which need a different grammar.
...
heuristic $9/326$ boundary into a sound map (\texttt{make forward-index}): \textbf{sound reach $11$}---delivering, as an exact reproducible
result, the reach lift the companion paper forecast for the PT closures---the $9$
...
asserting zero demotions across $\Theta$ levels). The loop's first step delivers it: registering the
witness-exact PT pentaketide$\to$naphthalene closure recovers the three deposited tetrahydroxynaphthalenes
(sound reach $11\to14$; they also become beam-reachable, $9\to12$), zero demotions, \emph{witness-first}---the
