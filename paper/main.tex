...
\date{May 2026 --- working draft (design reframe; integration pass)}
...
to $0.57$ across the iteration program.
...
modular-PKS precedent) and \emph{control} edits (the iteration program; the $0.57$ frontier)---scored
...
wall is thus a prediction ceiling read one way and a design frontier read the other. Because
structure elucidation in this field proceeds by heterologous expression and NMR with mass
...
transition policies. This stance answers a field diagnosis rather than ignoring it: Cox's review of
...
iteration program, the $0.57$ frontier)---under a \emph{literature-curated, super-additive} cost that
...
BiG-FAM \citep{bigslice} define the within-family neighbourhoods the cross-taxa transfer probe uses.
...
$100\%\!\to\!57\%$ cross-taxa collapse its one hard axis, and structural-similarity proxies are shown
...
measurement, a limitation and the contribution at once. We do not replace structure elucidation; we
build the sound triage layer between generative chemistry and protein design, and it knows where it
applies. The tools, benchmark, and analyses are open and reproducible.

\paragraph{Outlook.} The architecture is grammar-agnostic---observability and the three-state verdict
are shared across PKS, NRPS, and a typed hybrid---so deepening the family grammars (NRPS substrate
specificity, the tetramic-acid release, a learned transition policy for the highly-reducing wall) and
extending executor coverage widen the manifold without touching the shared layers. Two steps then
close the loop the planner opens: validating the observability layer on real LC-MS/MS spectra
(e.g.\ CFM-ID / GNPS), and turning the curated edit tiers into \emph{observation-updated probabilistic
realizability}---success distributions fit to wet-lab build outcomes---so that the
...
and \code{scripts/planner\_tree\_figure.py} (Figures~\ref{fig:worked}--\ref{fig:trees}). The tagged
